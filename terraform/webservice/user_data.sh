...
aws s3 cp "s3://${t_app_artefact_s3_bucket}/${t_app_artefact_file}" "${t_app_dir}/"

useradd springboot
chsh -s /sbin/nologin springboot
chown springboot:springboot "${t_app_dir}/${t_app_artefact_file}"
chmod 500 "${t_app_dir}/${t_app_artefact_file}"

ln -s "${t_app_dir}/${t_app_artefact_file}" "/etc/init.d/${t_service_instance_name}"
